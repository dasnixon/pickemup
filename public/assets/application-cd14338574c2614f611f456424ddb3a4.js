...
(function() {
  this.preference_app = angular.module('Preferences', ['ngResource', 'ui.compat']);

  this.skill_app = angular.module('Skills', ['ngResource', 'ui.compat']);

  this.jobListing = angular.module('JobListing', ['ngResource', 'ui.compat']);

  this.chunk = function(a, s) {
  ...
  return $scope.update = function() {
    JobListing.update({
      ...
      job_listing: {
        ...
        position_type: $scope.data.position_type,
        experience_level: $scope.data.experience_level,
        ...
        perks: $scope.data.perks,
        fulltime: $scope.data.fulltime,
        estimated_work_hours: $scope.data.estimated_work_hours,
        hiring_time: $scope.data.hiring_time,
        special_characteristics: $scope.data.special_characteristics,
        acceptable_languages: $scope.data.acceptable_languages
      }},
      function(response) {
...
(function() {
  preference_app.controller("PreferencesController", function($scope, $http, $location, $state, $stateParams, Preference) {
    $scope.preference = {};
    $scope.original = {};
    $scope.errors = [];
    $scope.success = '';
    ...
    if ($state.current.name === 'edit') {
      Preference.getPreference({
        ...
        return $scope.original = angular.copy($scope.preference);
      }, function(response) {
        return $scope.error_updating = 'Unable to fetch your preferences at this time.';
      });
    }
    $scope.$watch('preference', (function() {
      if (angular.equals($scope.preference, $scope.original)) {
        return $scope.dirty_message = '';
      } else {
        $scope.success = '';
        return $scope.dirty_message = 'You have made changes, make sure to click the \'Save\' button below.';
      }
    }), true);
    return $scope.update = function() {
      return Preference.update({
        ...
        preference: {
          ...
          expected_salary: $scope.preference.expected_salary ? $scope.preference.expected_salary.replace(/,/g, "") : void 0,
...
(function() {
  this.Skills = function($scope, Skill) {
    $scope.init = function(user_id) {
      return Skill.query({
        id: user_id
      }, function(data) {
        return $scope.profile = data[0].skills;
      });
    };
    return $scope.$watch('profile', function() {
      return $scope.rows = chunk($scope.profile, 6);
    });
  };

}).call(this);
(function() {
  jobListing.factory('JobListing', function($resource) {
    return $resource('/companies/:company_id/job_listings/:job_listing_id/:action', {
...
(function() {
  preference_app.factory('Preference', function($resource) {
    return $resource('/users/:user_id/:action/', {
      user_id: '@user_id',
      action: '@action'
    }, {
      'get': {
...
(function() {
  skill_app.factory('Skill', function($resource) {
    return $resource('/users/:id/skills', {
      id: '@id'
    }, {
      'query': {
        method: 'GET',
...
(function() {
  preference_app.config(function($stateProvider, $urlRouterProvider, $locationProvider) {
    $locationProvider.html5Mode(true);
  ...
  jobListing.config(function($stateProvider, $urlRouterProvider, $locationProvider) {
    $locationProvider.html5Mode(true);
    $stateProvider.state("edit", {
