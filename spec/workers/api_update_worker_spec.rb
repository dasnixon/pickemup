...
describe APIUpdateWorker do
  ...
  before :each do
    ...
    job_listing.should_receive(:api_update).and_return(api_response)
  end
  ...
  context 'when the API responds successfully' do
    it 'should not log anything' do
      ...
      APIUpdateWorker.new.perform(job_listing.id, job_listing.class.name)
    end
  end
  ...
  context 'when the API responds unsuccessfully' do
    ...
    it 'should log the model ID, class name, and response status' do
      ...
      APILogger.should_receive(:new).with('UPDATE').and_return(api_logger)
